...
@Composable
fun LirScheduleListScreen(navController: NavController, viewModel: LirSchedulesViewModel = hiltViewModel()){
    Scaffold(topBar = {
...
fun GetCutOffTimeList(
    viewModel: LirSchedulesViewModel,
...
val column10Weight = .08f

@Composable
fun FlightsTable(viewModel: LirSchedulesViewModel, navController: NavController) {
    val mContext = LocalContext.current
    ...
    LazyColumn(
        ...
        item {
            Row(
                ...
                TableCell(text = "Origin", weight = column1Weight, style = headerStyle)
                TableCell(text = "Dest", weight = column1Weight, style = headerStyle)
                TableCell(text = "ULD\nPositions", weight = column8Weight, style = headerStyle)
                TableCell(text = "Add LIR Data", weight = column10Weight, style = headerStyle)
            }
        }
        // data
        items(flightScheduleList.value) { flightScheduleModel->
            Row(
                ...
                TableCell(text = "${flightScheduleModel.originAirportCode}", weight =  column1Weight)
                TableCell(text = "${flightScheduleModel.destinationAirportCode}", weight =  column1Weight)
                TableCell(text = (flightScheduleModel.uldPositionCount.toString()), weight =  column8Weight)
                Box (modifier = Modifier.weight(column10Weight),
                    contentAlignment = Alignment.Center
                    ) {
                    IconButton(
                        onClick = {
